...
func TestIngestADRMembershipDeniesForeignRepository(t *testing.T) {
	cfg := appauth.Config{
	...
	handlers.IngestADR = commands.NewIngestADRsHandler(begin, fixed, &stubADRReader{})
	handlers.ListADRIngestRuns = queries.NewListADRIngestRunsHandler(begin)
	...
	listReq := httptest.NewRequest(http.MethodGet, "/v1/ingest/adr-runs?scope_kind=repository&scope_key=github.com/acme/other", nil)
