...
func TestIngestMembershipDeniesForeignRepository(t *testing.T) {
	cfg := appauth.Config{
	...
	handlers.IngestGit = commands.NewIngestGitHandler(begin, fixed, &stubGit{commits: []domain.GitCommitSnapshot{}})
	...
	listReq := httptest.NewRequest(http.MethodGet, "/v1/ingest/runs?scope_kind=repository&scope_key=github.com/acme/other", nil)
